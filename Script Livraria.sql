...
select count(idLivro) As QuantidadesDeLivros
from livro;
...
select e.nome as NomeEditora, I.titulo as TituloLivro
from Editora e
join livro i on e.idEditora = i.idEditora
order by e.nome desc;

select e.nome as NomeEditora, AVG(i.preco) as MediaPeco
from Editora e
join Livro i on e.idEditora = i.idEditora
group by e.nome;

select c.nome as NomeCliente, count(ip.idLivro) asquantidadeslivroscomprados
from cliente c
